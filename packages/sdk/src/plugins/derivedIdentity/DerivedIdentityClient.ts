...
export class DerivedIdentityClient implements IdentitySigner, KeypairSigner {
  ...
  async deriveFrom(
    ...
    this.originalSigner = originalSigner ?? this.ju.identity().driver();
  ...
  fund(amount: SolAmount) {
    this.assertInitialized();
    return this.ju.system().transferSol(
      {
  ...
  async withdrawAll() {
    this.assertInitialized();
    const balance = await this.ju
      .rpc()
      .getBalance(this.derivedKeypair.publicKey);
    const transactionFee = this.ju.utils().estimateTransactionFee();

...
export type CreateAccountInput = {
   ...
   * i.e. it will be equal to `await ju.rpc().getRent(space)`.
   */
  lamports?: SolAmount;
 ...
 *
 * ```ts
 * const transactionBuilder = await ju
 *   .system()
 *   .builders()
 *   .createAccount({ space: 100 }); // 100 bytes
 * ```
...
export const createAccountBuilder = async (
  ...
  const lamports = params.lamports ?? (await ju.rpc().getRent(space));
  assertSol(lamports);

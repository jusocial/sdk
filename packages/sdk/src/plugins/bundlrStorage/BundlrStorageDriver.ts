...
export class BundlrStorageDriver implements StorageDriver {
  ...
  constructor(ju: Ju, options: BundlrOptions = {}) {
    this._ju = ju;
    this._options = {
      providerUrl: ju.connection.rpcEndpoint,
      ...options,
    };
  }
  ...
  async getUploadPriceForFiles(files: JuFile[]): Promise<Amount> {
  ...
  async initBundlr(): Promise<WebBundlr | NodeBundlr> {
    ...
    const identity: Signer =
      this._options.identity ?? this._ju.identity();
...
const getJuFileTagsWithContentType = (
  file: JuFile
): JuFileTag[] => {
  if (!file.contentType) {
    return file.tags;
